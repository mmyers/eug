...
//        }
    ...
    private void initComponents() {
        ...
        javax.swing.JMenuItem viewCivilizationValueMenuItem = new javax.swing.JMenuItem();
        javax.swing.JMenuItem viewBarbarianPowerMenuItem = new javax.swing.JMenuItem();
        javax.swing.JMenuItem viewPopulationMenuItem = new javax.swing.JMenuItem();
        viewPopSplitMenuItem = new javax.swing.JMenuItem();
        ...
        yearSpinner.setModel(new SpinnerNumberModel(474, 0, 5000, 1));
        yearSpinner.setEditor(new javax.swing.JSpinner.NumberEditor(yearSpinner, "#####"));
        yearSpinner.setMaximumSize(new java.awt.Dimension(80, 30));
        ...
        viewCivilizationValueMenuItem.setAction(new DiscreteStepFilterAction("Civilization", "civilization_value", 0, 100, 5));
        viewMenu.add(viewCivilizationValueMenuItem);

        viewBarbarianPowerMenuItem.setAction(new DiscreteStepFilterAction("Barbarian power", "barbarian_power", 0, 9, 1));
        viewMenu.add(viewBarbarianPowerMenuItem);

        viewPopulationMenuItem.setAction(new DiscreteStepFilterAction("Population", "population", 0, 80, 4));
        viewMenu.add(viewPopulationMenuItem);

        viewPopSplitMenuItem.setAction(new PopSplitFilterAction());
        viewMenu.add(viewPopSplitMenuItem);
...
//    private transient Point lastClick = null;
    ...
    private void aboutMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_aboutMenuItemActionPerformed
        javax.swing.JOptionPane.showMessageDialog(this, "Rome History Editor\nVersion " + VERSION + "\nBy MichaelM");
...
//        provNameLabel.setText("Loading...");
        ...
        java.awt.EventQueue.invokeLater(
            ...
            public void run() {
                ...
                // Note the use of doClick() rather than directly invoking
                // actionPerformed(), in case the buttons are inactive.
                if (clickCount > 1) {
                    Class c = mapPanel.getMode().getClass();
                    if (c.equals(PoliticalMode.class) ||
...
//        lastClick = evt.getPoint();
    ...
    private void doMouseClick(final ProvinceData.Province p) {
        ...
        if (p.getId() != 0 && p.getId() < Integer.parseInt(Main.map.getString("sea_starts"))) {
        ...
        if (!(mode instanceof ProvinceMode)) {
            if (lastCountry == null) {
                ...
                // Do some neat stuff if this isn't a multi-mode
                if (mode instanceof CustomMode) {
                    ...
                    if (owner != null) {
                        String value = mapPanel.getModel().getHistString(owner, name);
                        if (value != null) {
                            mapPanel.setMode(new CustomCountryMode(mapPanel, name, value));
                            viewModeLabel.setText(name + " = " + value);
                            mapPanel.repaint();
                        }
                    }
...
//                    }
            ...
            if (lastProv.getId() > 0 && lastProv.getId() < Integer.parseInt(Main.map.getString("sea_starts"))) {
                /*if (mode instanceof ContinentMode) {
                    ...
                    String value = mapPanel.getModel().getHistString(p.getId(), name);
                    if (value != null) {
                        mapPanel.setMode(new CustomMode(mapPanel, name, value));
                        viewModeLabel.setText(name + " = " + value);
                        mapPanel.repaint();
                    }
...
//        lastClick = null;
    ...
    private void addCountryFilters() {
        ...
        final List<List<JMenu>> menus = new ArrayList<List<JMenu>>(26);
        for (int i = 0; i < 26; i++) {
            menus.add(new ArrayList<JMenu>());
        }
        ...
        for (int i = 0; i < menus.size(); i++) {
            ...
            if (menus.get(i).size() == 0) {
                JMenuItem dummy = new JMenuItem("(none)");
                dummy.setEnabled(false);
                menu.add(dummy);
            } else {
                for (JMenu m : menus.get(i)) {
                    menu.add(m);
                }
            }
    ...
    private void addCultureFilters() {
        ...
        Collections.sort(cultures.children, new ObjectComparator());
        
        final Comparator<String> listSorter = new StringComparator();
        
        for (GenericObject group : cultures.children) {
            GenericList all = group.getList("culture");
            all.sort(listSorter);
            
            JMenu groupMenu = new JMenu(Text.getText(group.name));
            
            StringBuilder pattern = new StringBuilder(all.size()*10).append('(');
            
            for (String culture : all) {
...
// <editor-fold defaultstate="collapsed" desc=" Generated Variables ">
    ...
    private javax.swing.JMenuItem viewPopSplitMenuItem;
